...
def update_cell_status(time, segment_id, densities, ctm_params, entry_flow, traffic_lights_df, traffic_lights_dict_states):
    ...
    for i in range(num_cells):  # iterate over all cells
        if i == 0:      # first cell
            ...
            inflow = min(CTMParameters().max_flow(), ctm_params.free_flow_speed * densities[i-1] * dt, ctm_params.wave_speed * (ctm_params.jam_density - densities[i]) * dt)
        ...
        if i == num_cells - 1:  # last cell
            ...
            outflow = min(CTMParameters().max_flow(), ctm_params.free_flow_speed * densities[i] * dt, ctm_params.wave_speed * (ctm_params.jam_density - densities[i+1]) * dt)
